use cosmwasm_std::{Api, Addr, BankMsg, Coin, CosmosMsg, Decimal, DepsMut, Response, SubMsg, Uint128, attr, Timestamp, Uint64};
...
use crate::commands::{place_bet, respond_bet, resolve_bet, liquidate_bet, withdraw_pending_bet};
use crate::state::{FlipSide, GameOutcome, HistoricalBet, OngoingBet, PendingBet, load_historical_bets, load_ongoing_bet, load_pending_bets, load_pending_bets_count};
use tefiluck::asset::Asset;

const MOCK_SIGNATURE: &'static str = "84dbedafb3b595f6d2d1c9719520f0e0a19fabd953afd4e5befc22a8fd0d7510";
...
fn test_proper_initialization() {
    ...
    assert_eq!(
        proper_instantiate(deps.as_mut()).is_ok(),
        true,
    );
}
...
fn test_place_bet() {
    ...
    let info = mock_info(
        "addr0001", 
        &[Coin{denom: "uusd".to_string(), amount: Uint128::new(1000000u128)}],
    );

    let place_bet_response = place_bet(
        deps.as_mut(), 
        env, 
        info, 
    ...
    assert_eq!(
        log_sender,
        &attr("sender", "addr0001".to_string()),
    );
...
fn test_place_bet_validation_error() {
    ...
    match place_bet(deps.as_mut(), env.clone(), info, MOCK_SIGNATURE.to_string(), 200).unwrap_err() {
    ...
    match place_bet(deps.as_mut(), env.clone(), info.clone(), MOCK_SIGNATURE.to_string(), 10).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
    ...
    match place_bet(deps.as_mut(), env.clone(), info.clone(), MOCK_SIGNATURE.to_string(), 10000).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
    ...
    let info = mock_info(
        "addr0001", 
        &[Coin{denom: "pinkguyasset".to_string(), amount: Uint128::new(1u128)}],
    );
    match place_bet(deps.as_mut(), env.clone(), info, MOCK_SIGNATURE.to_string(), 200).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
            "coin limits for provided asset not found".to_string(),
        ),
        _ => panic!("no error"),
    }
    ...
    let info = mock_info(
        "addr0001", 
        &[Coin{denom: "uusd".to_string(), amount: Uint128::new(1u128)}],
    );
    match place_bet(deps.as_mut(), env.clone(), info, MOCK_SIGNATURE.to_string(), 200).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
    ...
    match place_bet(deps.as_mut(), env.clone(), info, MOCK_SIGNATURE.to_string(), 200).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
            "bet with same id alreay exists".to_string(),
        ),
        _ => panic!("no error"),
    }
}
...
fn test_respond_bet() {
    ...
    let mut pending_bets = load_pending_bets(&deps.storage, &Addr::unchecked("addr0001")).unwrap();
    assert_eq!(
        pending_bets.find_by_id(&bet_id).is_err(),
        true,
    );
...
fn test_respond_bet_validation_error() {
    ...
    match respond_bet(deps.as_mut(), env.clone(), info, Addr::unchecked("addr0001"), "nf".to_string(), 0).unwrap_err() {
    ...
    let info = mock_info(
        "addr0002", 
        &[Coin{denom: "uusd".to_string(), amount: Uint128::new(2000000u128)}],
    );
    match respond_bet(deps.as_mut(), env.clone(), info, Addr::unchecked("addr0001"), bet_id.clone(), 0).unwrap_err() {
        ContractError::ResponderAssetMismatch {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
    ...
    match respond_bet(deps.as_mut(), env.clone(), info, Addr::unchecked("addr0001"), bet_id.clone(), 2).unwrap_err() {
        ContractError::Std(cosmwasm_std::StdError::GenericErr { msg, ..}) => assert_eq!(
            msg,
            "invalid flip side".to_string(),
        ),
        _ => panic!("no error"),
    }
}
...
fn test_resolve_bet() {
    ...
    let historical_bet = load_historical_bets(&deps.storage).unwrap().get(0).unwrap().clone();
    assert_eq!(
        historical_bet,
        HistoricalBet {
            id: bet_id.clone(),
...
fn test_resolve_bet_validation_error() {
    ...
    match resolve_bet(deps.as_mut(), env.clone(), info, bet_id.clone(), MOCK_PASSPHRASE.to_string()).unwrap_err() {
        ContractError::ExecuteWithoutFunds {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
    ...
    match resolve_bet(deps.as_mut(), env.clone(), info, bet_id.clone(), MOCK_PASSPHRASE.to_string()).unwrap_err() {
        ContractError::OnlyBetCreatorAllowedToResolve {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
    ...
    match resolve_bet(deps.as_mut(), env.clone(), info, bet_id.clone(), "pinkguy".to_string()).unwrap_err() {
        ContractError::SignatureMismatch {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
...
fn test_liquidate_bet_validation_error() {
    ...
    match liquidate_bet(deps.as_mut(), env, info, bet_id.clone()).unwrap_err() {
        ContractError::ForbiddenForBetCreatorToLiquidateHimself {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
    ...
    match liquidate_bet(deps.as_mut(), env, info, bet_id.clone()).unwrap_err() {
        ContractError::BetIsNotLiquidatableYet {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
    ...
    match liquidate_bet(deps.as_mut(), env, info, bet_id.clone()).unwrap_err() {
        ContractError::ResponderLiquidationGapIsNotPassedYet {} => assert_eq!(
            true,
            true,
        ),
        _ => panic!("no error"),
...
fn test_withdraw_pending_bet() {
    ...
    let response = withdraw_pending_bet(
        deps.as_mut(),
        info,
        bet_id.clone(),
    ).unwrap();
